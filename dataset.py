...
class C3D_h5_CRN(data.Dataset):
    def __init__(self, prefix="train"):
        if prefix=="train":
            self.file_path = '/data0/guodongyan/completion_dataset/our_data/train_data.h5'
        elif prefix=="val":
            self.file_path = '/data0/guodongyan/completion_dataset/our_data/valid_data.h5'
        elif prefix=="test":
            self.file_path = '/data0/guodongyan/completion_dataset/our_data/test_data.h5'
        else:
    ...
    def augment_cloud(self, Ps):
        ...
        # M = transforms3d.zooms.zfdir2mat(1)
        scale = np.random.uniform(1/1.5, 1)

        # if args.pc_augm_rot:
        #     angle = random.uniform(0, 2 * math.pi)
        #     M = np.dot(transforms3d.axangles.axangle2mat([0, 1, 0], angle), M)  # y=upright assumption
        if self.mirror > 0:  # mirroring x&z, not y
            if random.random() < self.mirror / 2:
    ...
    def __getitem__(self, index):
        ...
        if self.prefix is not "test":
            # partial = self.get_transform(partial)
            # complete = self.get_transform(complete)
